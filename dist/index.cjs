...
var index = declare_1(({ types: t }) => {
  const importFrom = /* @__PURE__ */ new Set();
  const registerFunctionNameNode = t.identifier(registerFunctionName);
  const registerNode = (path, state) => {
    const filename = state.filename;
    const { node, key, parent } = path;
    parent[key] = t.callExpression(
  ...
  return {
    ...
    visitor: {
      ObjectExpression: registerNode,
      ArrayExpression: registerNode,
      Program: {
        exit(path, { filename }) {
